...
/**
 * 2012-11-14:首台页面操作 wangujqw@gmail.com
 */
public class Index {
	...
	public void index() {
		...
		// 推荐商品(默认前几个进行滚动)
		out.set("recommendLst", M.me().get_pager_c(MMsg.WZ_CONTENT,
				MCnt.me().and("endtime", MEnum.GT_E, MDate.get_ymd()).toStr(),
				"recommended", 10,
				new Object[] { "ORDER BY is_recommend DESC" }));

		// 推荐广告
		out.set("adLst", M.me().get_pager_c(MMsg.WZ_ADVERTISING,
				MCnt.me().and("endtime", MEnum.GT_E, MDate.get_ymd()).toStr(),
				"aded", new Object[] { "ORDER BY is_recommend DESC" }));

		// 首页图片
		out.set("picLst", M.me().get_pager_c(
				MMsg.WZ_ADVERTISING,
				MCnt.me().and("endtime", MEnum.GT_E, MDate.get_ymd()).and_eq(
						"is_home_pic", "1").toStr(), "piced",
				new Object[] { "order by pubdate desc" }));
	...
	private MRecord get_comm() {
		...
		// 一级分类菜单表
		out.set("categoryLst", M.me().get_where(MMsg.XT_CATEGORY,
				MCnt.me().first_eq("parent_id", 0).toStr(),
				new Object[] { "order by sort asc" }));
		// 公告信息
	...
	public void submenu() {
		...
		// 二级菜单
		List<MRecord> submenuLst = M.me().get_where(MMsg.XT_CATEGORY,
				MCnt.me().first_eq("parent_id", main_id).toStr(),
				new Object[] { " order by sort asc" });
		List<MRecord> contentLst = null;
		// 取默认第一个一级菜单的列表
		if (!MCheck.isNull(submenuLst)) {
			MRecord category = submenuLst.get(0);
			String where = MCnt.me().first_eq("parent_id", main_id).and_eq(
					"category_id", category.get("id")).and_eq("is_audit", 1)
					.and("endtime", MEnum.GT_E, MDate.get_ymd()).toStr();
			contentLst = M.me().get_where(MMsg.WZ_CONTENT, where,
					"order by pubdate desc");
		}

		// 推荐商品
		List<MRecord> recommendLst = M.me().get_pager_c(
				MMsg.WZ_CONTENT,
				MCnt.me().and_eq("is_audit", 1).and_eq("is_recommend", 1)
						.and_eq("parent_id", main_id).and("endtime",
								MEnum.GT_E, MDate.get_ymd()).toStr(),
				"recommended", 10, new Object[] { "order by pubdate DESC" });

		MRecord out = get_comm();
		out.set("submenuLst", submenuLst).set("contentLst", contentLst).set(
				"recommendLst", recommendLst);
	...
	public void subcontent() {
		...
		List<MRecord> submenuLst = M.me().get_where(MMsg.XT_CATEGORY,
				...
				new Object[] { " order by sort asc" });
		String where = MCnt.me().first_eq("parent_id", main_id).and_eq(
				"category_id", parent_id).and_eq("is_audit", 1).and("endtime",
				MEnum.GT_E, MDate.get_ymd()).toStr();
	...
	public void ad() {
		...
		List<MRecord> submenuLst = M.me().get_where(MMsg.XT_CATEGORY,
				MCnt.me().first_eq("parent_id", main_id).toStr(),
				new Object[] { " order by sort asc" });
		MRecord contentObj = M.me().one_t(MMsg.WZ_ADVERTISING,
				MCnt.me().and_eq("id", content_id).toStr());
		MRecord out = get_comm();
	...
	public void content() {
		...
		MRecord contentObj = M.me().one_t(MMsg.WZ_CONTENT,
				MCnt.me().and_eq("id", content_id).toStr());

		// 明细图片
		List<MRecord> subImg = M.me().get_where(MMsg.WZ_CONTENT_MX,
				MCnt.me().first_eq("pid", content_id).toStr());
		MRecord out = get_comm();
		out.set("submenuLst", submenuLst).set("contentObj", contentObj).set(
				"subImg", subImg);
		out.set("page", "index_" + main_id);// 导航选中状态
		MR.me().setAttr(MMsg.OBJECT, out);
	...
	/**
	 * 如果选择了同步,则清理所有数据 1、数据库 2、文件图片
	 */
	private void clear() {
		String where = " 1=1 ";
		// 清理广告表
		M.me().delete(MMsg.WZ_ADVERTISING, where);
		// 清理数据内容表
		M.me().delete(MMsg.WZ_CONTENT, where);
		M.me().delete(MMsg.WZ_CONTENT_MX, where);

		// 清理菜单表
		M.me().delete(MMsg.XT_CATEGORY, where);

		// 清下图片文件
		String image_path = MPath.me().getWebRoot() + "upload/";
		deleteFile(new File(image_path));
	}

	private  void deleteFile(File file_path) {
		File file = file_path;
		if (file.isFile()) {
			file.delete();
